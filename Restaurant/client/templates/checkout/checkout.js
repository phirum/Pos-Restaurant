...
Template.restaurant_checkout.helpers({
  ...
  sale: function () {
    var s = Restaurant.Collection.Sales.findOne(FlowRouter.getParam(
      'saleId'));
    if (s != null) {
...
Template.restaurant_checkout.events({
  'click #save-without-pay': function () {
    ...
    Meteor.call('directUpdateSale', saleId, saleObj, function (er, re) {
      if (er) {
        ...
        Meteor.call('directUpdateSaleDetailsStatus', saleId, function (
          er, re) {
    ...
    alertify.confirm("Are you sure to cancel this order?")
      .set({
        onok: function (closeEvent) {
          Meteor.call('cancelSale', saleId, function (
            error) {
            if (error) {
  ...
  'click .btn-minus': function (e) {
    ...
    set.amount = (this.price * set.quantity) * (1 - this.discount /
      100);
    Meteor.call('updateSaleDetails', this._id, set, function (
      er, re) {
...
function addOrUpdateProducts(branchId, saleId, product, saleObj) {
  ...
  if (saleId == '') {
    ...
    if (saleDetail == null) {
      ...
      saleDetailObj.status = "Unsaved";
      saleDetailObj.note = "";
      Meteor.call('insertSaleDetails', saleDetailObj, function (
        error, result) {
      ...
      Meteor.call('directUpdateSaleDetailsStatus', saleId,
        function (er, re) {
          if (er) {
            ...
            // FlowRouter.go('/restaurant/tableView')
